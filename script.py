...
filtered_output_path = "/Users/santiagochang/Desktop/JSB/WBcel235/filtered_output.tsv"
...
def construct_chromosome_dictionaries(fasta_file):
    # status update
    print('Constructing nucleotide sequence dictionary.')
    print('Constructing chromosome label dictionary.')
...
# input: BigWig file
# output: chromosome ID to phyloP dictionary
def construct_phylop_dictionary(bigwig_file):
    # status update
    print('Constructing phyloP scores dictionary.')
    # open the BigWig file containing phyloP scores
    file = pyBigWig.open(bigwig_file)
    # initialize dictionary
    chrom_id_to_phylop = {}
    # for each chromosome, access all phyloP scores, and map chromosome ID to phyloP scores in dictionary
    for chrom, length in file.chroms().items():
        scores = file.values(chrom, 0, length)
        chrom_id_to_phylop[chrom] = scores
    # close the BigWig file
    file.close()
    # return chromosome ID to phyloP scores dictionary
    return chrom_id_to_phylop
...
# input: phyloP dictionary, chromosome label, start index, stop index, strand direction
# output: list of phyloP scores
def get_phylop_scores(phylop_dictionary, chromosome_label, start, stop, strand):
    # extract scores defined by chromosome and indices from phyloP dictionary
    scores = phylop_dictionary[chromosome_label][start-1:stop]
...
def construct_data_frame(gtf_data_frame, nucleotide_dictionary, chromosome_dictionary, phylop_dictionary):
    ...
    for index in range(len(gtf_data_frame)):
        ...
        # if row's feature is an exon coming from a protein coding gene and mRNA transcript
            ...
            phylop_scores.extend(get_phylop_scores(phylop_dictionary, 'chr' + chrom_label, start, end, strand))
...
# from phyloP scores file (BigWig), generate phyloP scores dictionary
phylop_dict = construct_phylop_dictionary(phylop_file)
...
processed_df = construct_data_frame(gtf_df, nuc_dict, chrom_dict, phylop_dict)
processed_df.to_csv(output_path, sep = '\t', index = False)

# filter out genes with transcripts lacking 5' UTR and write to .tsv file
filtered_df = processed_df[(processed_df['cds'] != processed_df['cdna']) & (processed_df['cds_start_in_mRNA'] != 0)]
